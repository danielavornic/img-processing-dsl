...
class CommandExecutor:
    ...
    def execute(self):
        ...
        if any("convert" in command for command in self.command_results):
            convert_command = [
                command for command in self.command_results if "convert" in command][0]
        ...
        for index, command_result in enumerate(self.command_results):
            ...
            if command == "rotate":
                self.img = img_basic_operations.rotate(
                    self.img, int(parameters[0]))
                if is_last_command:
                    img_save_loader.save(self.img, self.image_path)
            elif command == "resize":
                self.img = img_basic_operations.resize(
                    self.img, int(parameters[0]), int(parameters[1]))
                if is_last_command:
                    img_save_loader.save(self.img, self.image_path)
            elif command == "crop":
                self.img = img_basic_operations.crop(self.img, int(parameters[0]), int(
